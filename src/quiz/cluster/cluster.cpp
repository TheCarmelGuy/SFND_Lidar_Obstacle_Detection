...
void proximity(const std::vector<std::vector<float>>& points,int index, 
    std::vector<int>& isMarked, std::vector<int>& cluster,KdTree* tree,
    float distanceTol)
{
    ...
    KdTree* tree, float distanceTol)
{
  ...
  std::vector<int> isMarked(points.size(), 0); 
  ...
  for(int i = 0; i < points.size(); i++)
  {
    if(isMarked[i] == 0)
    {
      ...
      proximity(points,i, isMarked, currentCluster,tree,distanceTol); 
